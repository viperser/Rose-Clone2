...
require("./config.js")
const { default: RoseMwolConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
...
async function startRoseMwol() {
    ...
    console.log(color('✔', 'cyan'), color('Rose Mwol Is Online...', 'pink'))
...
store.bind(RoseMwol.ev)
    ...
    let pa7rick = await RoseMwol.sendContact(callerId, global.owner)
    RoseMwol.sendMessage(callerId, { text: `Automatic Block System!\nDon't Call Bot!\nPlease Ask Or Contact The Owner To Unblock You!`}, { quoted : pa7rick })
    await sleep(8000)
...
RoseMwol.sendMessage(anu.id, buttonMessage, {quoted:unicorndoc})
...
// Setting
	...
	RoseMwol.sendMessage(jid, { contacts: { displayName: `${list.length} Contact`, contacts: list }, ...opts }, { quoted })
    }
    ...
    RoseMwol.ev.on('connection.update', async (update) => {
        ...
        let reason = lastDisconnect.error ? lastDisconnect?.error?.output.statusCode : 0;
            ...
            else if (reason === DisconnectReason.connectionClosed) { console.log("Connection closed, reconnecting...."); startRoseMwol(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("Connection Lost from Server, reconnecting..."); startRoseMwol(); }
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); process.exit(); }
            else if (reason === DisconnectReason.loggedOut) { console.log(`Device Logged Out, Please Delete Session and Scan Again.`); process.exit(); }
            else if (reason === DisconnectReason.restartRequired) { console.log("Restart Required, Restarting..."); startRoseMwol(); }
            else if (reason === DisconnectReason.timedOut) { console.log("Connection TimedOut, Reconnecting..."); startRoseMwol(); }
    ...
    RoseMwol.send5ButImg = async (jid , text = '' , footer = '', img, but = [], thumb, options = {}) =>{
        let message = await prepareWAMessageMedia({ image: img, jpegThumbnail:thumb }, { upload: RoseMwol.waUploadToServer })
        ...
        imageMessage: message.imageMessage,
            ...
            RoseMwol.relayMessage(jid, template.message, { messageId: template.key.id })
    }
    ...
    RoseMwol.sendButtonText = (jid, buttons = [], text, footer, quoted = '', options = {}) => {
        ...
        RoseMwol.sendMessage(jid, buttonMessage, { quoted, ...options })
    }
     ...
     * @returns 
     */
    RoseMwol.sendText = (jid, text, quoted = '', options) => RoseMwol.sendMessage(jid, { text: text, ...options }, { quoted })
	...
	let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        return await RoseMwol.sendMessage(jid, { image: buffer, caption: caption, ...options }, { quoted })
    }
    ...
    RoseMwol.sendVideo = async (jid, path, caption = '', quoted = '', gif = false, options) => {
        let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        return await RoseMwol.sendMessage(jid, { video: buffer, caption: caption, gifPlayback: gif, ...options }, { quoted })
    }
    ...
    RoseMwol.sendAudio = async (jid, path, quoted = '', ptt = false, options) => {
        let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        return await RoseMwol.sendMessage(jid, { audio: buffer, ptt: ptt, ...options }, { quoted })
    }
     ...
     * @returns 
     */
    RoseMwol.sendTextWithMentions = async (jid, text, quoted, options = {}) => RoseMwol.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
    ...
    RoseMwol.sendImageAsSticker = async (jid, path, quoted, options = {}) => {
        ...
        await RoseMwol.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted })
        return buffer
    }
	...
	RoseMwol.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
       ...
       await RoseMwol.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ...options })
    ...
    RoseMwol.copyNForward = async (jid, message, forceForward = false, options = {}) => {
        ...
        await RoseMwol.relayMessage(jid, waMessage.message, { messageId:  waMessage.key.id })
        return waMessage
    }
...
//sendlistmsg by xeon
        ...
        RoseMwol.sendMessage(jid, listMes, { quoted: quoted })
        }
    ...
    RoseMwol.getFile = async (PATH, save) => {
        ...
        let message = await prepareWAMessageMedia({ video: gif, gifPlayback: true }, { upload: RoseMwol.waUploadToServer })
        ...
        let message = await prepareWAMessageMedia({ video: vid }, { upload: RoseMwol.waUploadToServer })
        var template = generateWAMessageFromContent(jid, proto.Message.fromObject({
        templateMessage: {
        hydratedTemplate: {
        videoMessage: message.videoMessage,
    ...
    RoseMwol.sendFile = async(jid, PATH, fileName, quoted = {}, options = {}) => {
        let types = await RoseMwol.getFile(PATH, true)
        ...
        await RoseMwol.sendMessage(jid, { [type]: { url: pathFile }, mimetype, fileName, ...options }, { quoted, ...options })
        return fs.promises.unlink(pathFile)
    }
    RoseMwol.parseMention = async(text) => {
